...
describe('saves', () => {
  ...
  it('migrates v16 saves to the corrected access map and surveyed coastline', () => {
    const v16 = testState(568);
    v16.saveVersion = 16;
    v16.mapKnowledge.surveyedCells = [];

    const migrated = deserialize(serialize(v16), {
      locationDefs: TEST_LOCATIONS,
      mapRegionDefs: MAP_REGIONS,
      mapFeatureDefs: MAP_FEATURES,
    });
  ...
  it('migrates fresh v17 saves by removing the old auto-charted starter route', () => {
    const v17 = testState(569);
    v17.saveVersion = 17;
    v17.turn = 1;
    v17.phase = 'assignment';
    v17.expeditions = [];
    v17.pendingEvents = [];
    v17.mapKnowledge.surveyedCells = [
      ...v17.mapKnowledge.surveyedCells,
      mapCellIndex({ x: 0.5, y: 0.1 }),
    ];

    const migrated = deserialize(serialize(v17), {
      locationDefs: TEST_LOCATIONS,
      mapRegionDefs: MAP_REGIONS,
      mapFeatureDefs: MAP_FEATURES,
    });
  ...
  it('migrates v23 saves: no old pool ever had a settled group under tension, friction backfills empty', () => {
    const v23 = JSON.parse(serialize(testState())) as Record<string, unknown>;
    v23.saveVersion = 23;
    delete (v23.residents as Record<string, unknown>).friction;

    const migrated = migrate(v23 as unknown as GameState);
    expect(migrated.saveVersion).toBe(24);
    expect(migrated.residents.friction).toEqual({});
  });
